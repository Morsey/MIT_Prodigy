...
def setup_conn(port, accept_handler, interface=None):
    global listen_s
    ...
    if not interface:
        ...
        iface = network.WIZNET5K(spi, Pin(17), Pin(20))  # spi,cs,reset pin
        iface.active(True)
...
def start(interface=None, port=8266, password=None, accept_handler=accept_conn):
    global static_host
    stop()
    ...
    if not interface:
        ...
        s = setup_conn(port, accept_handler, interface=interface)
