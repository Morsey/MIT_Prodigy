...
class NFC:
    def __init__(self, nss_pin, rst_pin, bsy_pin, card_reader_id = None, sck=10, mosi=11, miso=12):
        ...
        self._timeout = 100  # set timeout to 200 ms
...
card_index = 0
nfc = NFC(nss_pins_numbers[card_index], rst_pins_numbers[card_index], busy_pins_numbers[card_index], card_reader_id=card_index, sck=2, mosi=3, miso=4)
nfc.begin()
nfc.reset()

        
print(f"card : {card_index}")
print(f"firmware: {nfc.get_firmware()}")
